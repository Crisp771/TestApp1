using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.Serialization;
using System.ServiceModel;
using System.Text;
using TestAppData;
using TestAppObjects;

namespace TestAppWCF
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "SiteService" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select SiteService.svc or SiteService.svc.cs at the Solution Explorer and start debugging.
    public class SiteService : ISiteService
    {
        public IList<Site> GetSites()
        {
            return new SiteObjects().GetSites();
        }

        public Site GetSiteByID(int siteID)
        {
            return (Site)(new SiteObjects().GetSiteByID(siteID));
        }

        public Site CreateSite(Site site)
        {
            return (Site) (new SiteObjects().CreateSite(site));
        }

        public Site UpdateSite(Site site)
        {
            return (Site) (new SiteObjects().UpdateSite(site));
        }
        public void DeleteSite(int siteID)
        {
            new SiteObjects().DeleteSite(siteID);
        }
        public IList<SICLookup> GetSICLookups()
        {
            return new SICLookupObjects().GetSICLookups();
        }
    }
}
